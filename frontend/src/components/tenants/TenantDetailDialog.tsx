...
const ProfileInfoItem = ({ icon, label, value }: { icon?: React.ComponentType<{ className?: string }>, label: string, value: string }) => (
...
const StatItem = ({ label, value, valueColor = 'text-blue-600' }: { label: string, value: number, valueColor?: string }) => (
...
const TabButton = ({ label, tab, activeTab, setActiveTab }: { label: string, tab: string, activeTab: string, setActiveTab: (tab: string) => void }) => (
    <button
...
export function TenantDetailDialog({
  ...
  const getStatusBadge = (status: string) => {
    ...
    return variants[status as keyof typeof variants] || { className: 'bg-slate-100 text-slate-800', label: status };
  };
                ...
                <div className="lg:col-span-1 lg:border-r border-slate-200 p-6 space-y-6 bg-white lg:bg-slate-50">
                    <div>
                        ...
                        <div className="space-y-4">
                            <ProfileInfoItem icon={Phone} label="Số điện thoại" value={tenantDetail?.phone || 'N/A'} />
                            <ProfileInfoItem icon={CreditCard} label="Số CCCD" value={tenantDetail?.idCard || 'N/A'} />
                            ...
                            <ProfileInfoItem icon={MapPin} label="Quê quán" value={tenantDetail?.hometown || 'N/A'} />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        {activeTab === 'residency' && (
                            <ResidencyRecordList
                                ...
                                tenantName={tenantDetail?.fullName || 'N/A'}
